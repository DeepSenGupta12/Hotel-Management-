...
if(isset($_GET['delid']))
{
    $tourImages = tourImages($mysqli,$_GET['delid']);
    unlink('../assets/tour/'.$tourImages['images']);
    $delete = mysqli_query($mysqli,"delete from `tour_images` where `id` = '".$tourImages['id']."'");
    header('Location: '.$link.'/tour_images/'.$tourImages['t_id']);
}
$tourPlace = tourPlace($mysqli,$id);
$fetchData_rec = mysqli_query($mysqli,"select * from `tour_images` where `t_id`='".$tourPlace['id']."' order by `id` desc");
                            ...
                            <div class="card card-rounded">
                              <div class="card-body">
                                <div class="d-sm-flex justify-content-between align-items-start">
                                  <div>
                                    <h4 class="card-title card-title-dash">GALLERY IMAGES <span style="color:green">[<?php echo strtoupper($tourPlace['title']);?>]</span></h4>
                                   <p class="card-subtitle card-subtitle-dash">Total Images [<?php echo $fetchCount;?>]</p>
                                  </div>
                                  <div>
                                    <a href="<?php echo $link;?>/tour_add_images/<?php echo $tourPlace['id'];?>" class="btn btn-primary btn-lg text-white mb-0 me-0" type="button"><i class="fa fa-plus"></i>ADD IMAGES</a>
                                    <a href="<?php echo $link;?>/tour_view" class="btn btn-warning btn-lg text-white mb-0 me-0" type="button"><i class="fa fa-eye"></i>VIEW TOUR PLACES</a>
                                  </div>
                                </div>
                                    ...
                                    ?>
                                      <tr>
                                        ...
                                        <td>
                                          <div class="d-flex ">
                                            <div>
                                            <img style="width: 150px;height: auto;" src="<?php echo $sitelink;?>/assets/tour/<?php echo $fetchData_Res['images'];?>" alt="">
