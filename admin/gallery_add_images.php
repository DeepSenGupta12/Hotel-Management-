    ...
    if($csrf_token == $token)
    {
        if($_POST['Submit'] == "Add Images")
        {
            ...
            foreach($_FILES["files"]["tmp_name"] as $key=>$tmp_name) 
            {
                ...
                move_uploaded_file($_FILES["files"]["tmp_name"][$key],"../assets/gallery/".$imageName);
                resize(750,"../assets/gallery/".$imageName,"../assets/gallery/".$imageName);
                $galleryIns = mysqli_query($mysqli,"insert into `gallery_images`(`g_id`,`images`) values('".$galleryCat['id']."','".$imageName."')");
                   
               
            }
            $_SESSION['task'] = "success";
            $_SESSION['msg'] = "Image Added Successfully";
            header('Location: '.$link.'/gallery_images/'.$galleryCat['id'].'?task=success'); 
                          ...
                          <div class="col-12 col-lg-4 col-lg-12 grid-margin stretch-card">
                            <div class="card card-rounded">
                              ...
                              <div class="d-sm-flex justify-content-between align-items-start">
                                  <div>
                                    <h4 class="card-title card-title-dash">ADD MORE IMAGES&nbsp;<span style="color:green">[<?php echo strtoupper($galleryCat['title']);?>]</span></h4>
                                   <p class="card-subtitle card-subtitle-dash">( GALLERY ADD AND UPDATE )</p>
                                  </div>
                                  
                                  <div>
                                    <a href="<?php echo $link;?>/gallery_images/<?php echo $galleryCat['id'];?>" class="btn btn-primary btn-lg text-white mb-0 me-0" type="button"><i class="fa fa-eye"></i>&nbsp;VIEW IMAGES</a>
                                  </div>
                                </div>
