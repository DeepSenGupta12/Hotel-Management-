...
list($news,$pagename,$id) = explode("/",trim($_SERVER['REQUEST_URI'],"/"));
if(isset($id))
{
...
if(isset($_GET['delid']))
{
    $newsMoreImages = newsMoreImages($mysqli,$_GET['delid']);
    unlink('../images/news/'.$newsMoreImages['images']);
    $delete = mysqli_query($mysqli,"delete from `news_images` where `id` = '".$_GET['delid']."'");
}

$fetchData_rec = mysqli_query($mysqli,"select * from `news_images` where `nid`='".$newsDetails['id']."' order by `id` desc");
                            ...
                            <div class="card card-rounded">
                              <div class="card-body">
                                <div class="d-sm-flex justify-content-between align-items-start">
                                  <div>
                                    <h4 class="card-title card-title-dash">NEWS IMAGES</h4>
                                   <p class="card-subtitle card-subtitle-dash">List of User</p>
                                  </div>
                                  <div>
                                    <a href="<?php echo $link;?>/news_add_images/<?php echo $newsDetails['id'];?>" class="btn btn-primary btn-lg text-white mb-0 me-0" type="button"><i class="fa fa-plus"></i>ADD IMAGES</a>
                                  </div>
                                </div>
                                    ...
                                    ?>
                                      <tr>
                                        ...
                                        <td>
                                          <div class="d-flex ">
                                            <div>
                                            <img style="width: 150px;height: auto;" src="<?php echo $sitelink;?>/images/news/<?php echo $fetchData_Res['images'];?>" alt="">
